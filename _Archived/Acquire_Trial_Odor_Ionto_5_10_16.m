function data = Acquire_Trial_Odor_Ionto(expNumber,trialDuration, iontoDuration, odor, valveID, Istep)
...
[data,n] = acquisitionSetup(expNumber, trialDuration, iontoDuration, [], odor, valveID, Istep, []);
...
%     %data(n).Ihold = Ihold;    
    ...
    % Set up current step
    if ~isempty(Istep)
        preStepOut = zeros(sampRate, 1); % = ones(sampRate,1) * Ihold/2000
        stepOut = ones(sampRate./2, 1) * Istep/2000; % = (ones(sampRate./2, 1) * (Ihold + Istep)/2000) 
        postStepOut = zeros(sampRate * (sum(trialDuration) - 1.5),1); %= zeros(sampRate * (sum(trialDuration) - 1.5),1) * Ihold/2000
        Icommand = [preStepOut; stepOut; postStepOut];
        if ~isempty(iontoDuration)
           Icommand((iontoDuration(1)+1)*sampRate:(iontoDuration(1)+1.5)*sampRate) = Istep/2000; %(Ihold + iStep)/2000 
        end
    end
...
%       5  Amplifier Mode
    ...
    if stimOn
        ...
        s.addDigitalChannel('Dev1', 'port0/line0', 'OutputOnly');       % Iontophoresis command                              
