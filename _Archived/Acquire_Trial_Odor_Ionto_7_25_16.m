...
data(n).acquisition_filename = mfilename('fullpath');       % saves name of mfile that generated data
    ...
    if ~isempty(Istep)
        preStepOut = ones(sampRate * data(n).stepStartTime, 1) * Ihold/2000;
        stepOut = ones(sampRate * data(n).stepLength, 1) * (Ihold + Istep)/2000;
        postStepOut = ones(sampRate * (sum(trialDuration) - (data(n).stepStartTime + data(n).stepLength)), 1) * Ihold/2000;
        ...
        if ~isempty(iontoDuration)
            ...
            Icommand(iontoStepStart:iontoStepEnd) = (Ihold + Istep)/2000;
        end
    else
        Icommand = ones(sampRate*sum(trialDuration),1) * Ihold/2000;
    end
