...
%% set trial parameters 
    ...
    stimVolts = 5;
    sampRate = 10000;
    testPulseLength = 0.1;                                      % Length of vHold test pulse (sec)
...
%       5  Amplifier Mode
    ...
    if strcmp(ampMode, 'Vclamp')
        ...
        s.queueOutputData([cleanValveOut, pinchValveOut, extCommand]);
   ...
   elseif gainReading >= 5.34 && gainReading < 5.85
    ...
    if strcmp(ampMode, 'Vclamp')
        ...
        I1 = mean(scaledOut(length(prePulse)-(.05 * sampRate) : length(prePulse)-(.01 * sampRate)));
        ...
        data(n).Ra = 1000 * (dV / dIt);      % Calculate access resistance (MOhm)
        data(n).Rin = 1000 * (dV / dIs);     % Calculate input resistance (MOhm)
