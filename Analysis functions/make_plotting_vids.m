...
%
% INPUTS:
%       expData = entire imaging data structure for the experiment in question.
%
...
% try
    for iTrial = 1:length(expData.expInfo);
        ...
        if ~isempty(dir(fullfile(parentDir, strDate, trialStr, '*tif*'))) && isempty(dir(fullfile(savePath, [trialStr, '_*']))) 
            ...
            blTemp = get_trials(getTrials(expData, iTrial), iTrial);
            ...
            for iFrame = 1:size(myMovie, 3)
                ...
                plot_traces(ax, blTemp, fTemp, currVm', [0 0 1], annotLines, annotColors);
