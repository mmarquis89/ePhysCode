function [data, current, scaledOut, tenVm] = acquisitionPostProcessing(data, rawAcqData, trialNum)
% ================================================================================================
...
if ~isempty(data.Istep)
    data.Rin = calcRinput(scaledOut, data.sampratein, data.Istep, data.stepStartTime, data.stepLength, 0.5);
else
...
if trialNum == 1
    data.Rpipette = pipetteResistanceCalc(current);
else
    data.Rpipette = [];
end
